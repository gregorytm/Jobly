...
const { BadRequestError, ExpressError, ForbiddenError } = require("../../express-jobly/expressError");
...
function sqlForPartialUpdate(dataToUpdate, propertyToColumnNameMap={}) {
  ...
  const cols = keys.map((dataPropertyName, idx) =>
      `"${propertyToColumnNameMap[dataPropertyName] || dataPropertyName}"=$${idx + 1}`,
  );
